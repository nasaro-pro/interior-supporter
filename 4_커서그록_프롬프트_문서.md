...
`docs/SITE_DESIGN.md` ← 「② 총 사이트 설계문서」(Word를 마크다운으로 변환)

`docs/PROMPTS.md` ← 이 문서

저장소 루트에 `AGENTS.md`를 만들고 **1장 헌법**을 그대로 붙여넣는다.

Cursor에서 모델은 **Grok 코드 계열**, Composer는 **Agent 모드**로 쓴다.
...
| 8   | `grep -rn "better-auth" src/ | grep -v "src/lib/auth"`                     |
...
```
docs/SITE_DESIGN.md 3.C 와 ARCHITECTURE.md 부록 C 를 참고해서
...
```
docs/SITE_DESIGN.md 3.D 대로 PM 워크스페이스를 만들어줘.
...
```
docs/SITE_DESIGN.md 4.3~4.5, 5.1, 5.2 와 ARCHITECTURE.md 8.2, 8.4~8.6, 8.8 을 참고해서
프로젝트 콘텐츠 화면을 구현해줘.
...
```
docs/SITE_DESIGN.md 3.E 와 ARCHITECTURE.md 4.1·9.1절을 읽고 고객 화면을 구현해줘.
...
```
docs/SITE_DESIGN.md 4.2 와 ARCHITECTURE.md 9장을 읽고
...
```
docs/SITE_DESIGN.md 3.B, 4.6 대로 /platform 하위를 구현해줘.
