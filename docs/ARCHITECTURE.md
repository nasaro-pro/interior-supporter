...
| 4 | **역할은 멤버십으로 부여.** 한 계정이 여러 업체에 다른 역할로 속할 수 있다. 시공팀 포함 개인 계정. 실제 프로젝트 접근은 ProjectAssignment 또는 ProjectAccess | 6.4, 7.4 |
| 5 | **고객 화면은 고정 메뉴 + 홈만 CMS.** 7개 상세는 고정 화면, 프로젝트 홈만 블록 구성. 1차 홈 편집은 12컬럼+프리셋 | 9장 |
| 6 | **요금제는 설계만, 적용 보류.** 전 기능 무료 개방, 체험 기간 개념 없음 | 13장 |
| 7 | **알림은 이메일 우선, 채널 확장 가능 구조** | 11장 |
| 8 | **인프라는 해외(Vercel/Neon/R2) + 국외 이전 고지·동의** | 15장 |
| 9 | **이미지는 앱이 권한 확인 후 중계.** 버킷은 완전 비공개. SKP/DWG는 다운로드만 | 10장 |
| 10 | **파일럿 우선.** 안전한 기반+핵심 흐름 후 2~4주 실사용. 전 기능 완성 후 공개가 아니다 | 21장 |
...
| 버전 | 일자 | 핵심 변경 |
|---|---|---|
| v3.2 | 2026-09-15 | 2차 설계안: `field_worker`, ProjectAssignment, 견적·미팅·요청원문·현장일지, RLS 파일럿 전 필수, 파일럿 출시(ADR-13) |
...
| ADR-13 | **파일럿 우선 출시.** 기반+핵심 흐름 후 2~4주 실사용 | 규제상 전 기능 동시 출시가 강제될 때 |
| ADR-14 | 프로젝트 접근은 Membership과 별도 ProjectAssignment | 없음 — 시공팀 기간 회수의 토대 |
| ADR-15 | PostgreSQL RLS를 파일럿 전 이중 격리로 적용 | 없음 — 보안 원칙 |
...
export const membershipRoleEnum = pgEnum('membership_role', ['company_admin', 'project_manager', 'field_worker'])
...
export const scheduleTypeEnum = pgEnum('schedule_type', ['process', 'visit', 'confirmed', 'meeting'])
...
/** 일반 대화 vs 확정 지시 (7.6절) */
export const assignmentRoleEnum = pgEnum('assignment_role', ['designer', 'field_worker'])
export const workLogStatusEnum = pgEnum('work_log_status', ['started', 'in_progress', 'done'])
export const requestStatusEnum = pgEnum('request_process_status', ['open', 'in_progress', 'done'])
...
export const auditActionEnum = pgEnum('audit_action', [
  ...
  'cross_tenant_query',
  'assignment_grant', 'assignment_revoke', 'field_log_complete',
  'estimate_publish', 'meeting_publish', 'request_create',
])
```
...
```ts
// src/modules/assignment/schema.ts — 프로젝트·역할·기간 배정 (2차)
...
```ts
// 견적·미팅·요청·현장일지는 각각 estimate / meeting / request / field-log 모듈. 컬럼은 코드 스키마가 출처다.
```
...
| I12 | `project_manager` 는 본인 담당(manager_id 또는 designer 배정) 프로젝트만 편집한다. `field_worker` 는 `assertFieldAssigned` 로만 현장 기록을 쓴다 | `assertStaffOwnsProject()` · `assertFieldAssigned()` (6.4) |
...
`assertStaffOwnsProject`는 `company_admin`은 통과시키고, `project_manager`는 `projects.manager_id === ctx.userId` 또는 활성 designer 배정이 있을 때만 통과시킨다. `field_worker`는 거부하고 `assertFieldAssigned`를 쓴다.
...
### 6.6 PostgreSQL RLS (파일럿 전 필수)

애플리케이션 계층과 함께 RLS를 외부 파일럿 전에 켠다. `platform`·`system` 만 `app.bypass_rls=on`.

```sql
-- drizzle/manual/0003_rls.sql
...
트랜잭션 시작 시 `lib/db/rls.ts` 가 `SET LOCAL` 을 주입한다.
